...
void syspool_init_specific_size(uint32_t size)
{
    ...
    if(hal_overlay_is_used()){
        ...
        if(overlay_addr == NULL){
            ...
            HEAP_TRACE(2,"syspool_init1: %p,0x%x",__cp_mem_pool_start__,syspool_size);
        }else{
            memset(overlay_addr,0,syspool_size);
            HEAP_TRACE(2,"syspool_init2: %p,0x%x",overlay_addr,syspool_size);
        }
    }else{
        memset(__cp_bss_sram_end,0,syspool_size);
        HEAP_TRACE(2,"syspool_init3: %p,0x%x",__cp_bss_sram_end,syspool_size);
    }
#else
    memset(__mem_pool_start__,0,syspool_size);
    HEAP_TRACE(2,"syspool_init4: %p,0x%x", __mem_pool_start__,syspool_size);
#endif
}
