...
BUILD_CUSTOMER_CFG="A2DP_LHDCV5_ON=1 A2DP_LHDC_ON=1 A2DP_LHDC_V3=1 A2DP_LC3_ON=1 A2DP_LDAC_ON=1 BES_OTA=1 \
                 ...
                 BT_SVC_MODULE_LEA_ENABLED=1 RAMCP_SIZE=0x80000 GATT_OVER_BR_EDR=1 BES_OTA=1 BES_LIB_DIR=lib/bes/best1503/PRO_0004\
...
then
    ...
    make T=prod_test/ota_copy CHIP=$CHIPID OTA_BIN_COMPRESSED=1 BES_LIB_DIR=lib/bes/best1503/PRO_0004 DEBUG=1 -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
elif [[ "$COMMAND" == 'lst' ]];
then
    ...
    make T=prod_test/ota_copy CHIP=$CHIPID OTA_BIN_COMPRESSED=1 BES_LIB_DIR=lib/bes/best1503/PRO_0004 DEBUG=1 -j64 ||{ echo "$LINENO command failed"; exit 1; }
fi
